<template>
	<view class="cl-li">
		...
		<view class="cl-li__search" v-if="filterable">
			<cl-input v-model="keyWord" :border="false" prefix-icon="cl-icon-search" round placeholder="搜索关键字" clearable @search="onSearch"></cl-input>
		</view>

		<!-- 滚动视图 -->
		<scroll-view class="cl-li__scroller" scroll-y enable-back-to-top scroll-with-animation :scroll-into-view="`index-${label}`"
		 @scroll="onScroll">
			<!-- 追加内容到头部 -->
		...
		<view class="cl-li__bar">
			<view class="cl-li__bar-list" @touchmove.stop.prevent="barMove" @touchend="barEnd">
				<template v-for="(item, index) in flist">
					<view :class="['cl-li__bar-block', curr.label == item.label ? 'is-active' : '']" :key="index" :id="index"
